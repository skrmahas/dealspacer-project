...
function normalizeExtractedMetric(
  metric: ExtractedMetric,
  snapshot: PreviewSnapshot | null,
): number {
  return normalizeMetricValueToEur(
    { ...metric, label: metric.originalLabel ?? metric.label },
    snapshot,
  ) ?? metric.value ?? 0;
}
    ...
    const target =
      metric?.value != null
        ? metric.normalizedValue ??
          normalizeExtractedMetric(metric, snapshot)
        : null;
...
function normalizeTrendValue(
  ...
  return normalizeMetricToEur(value, "EUR", "Revenue");
}
...
export function resolvePreviewMetric(
  ...
  if (m?.value != null) {
    return m.normalizedValue ?? normalizeExtractedMetric(m, snapshot);
  }
