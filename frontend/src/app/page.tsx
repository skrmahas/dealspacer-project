    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-lg">
        ...
        <p className="text-zinc-400 text-center mb-10 text-sm">
          Upload an earnings report — get a structured financial summary.
        </p>
        ...
        >
          ...
          {uploading ? (
            ...
            <>
              ...
              <span className="text-zinc-600 text-xs mt-1">PDF only · Max 50MB</span>
            </>
          )}
        </label>
          ...
          <div className="mt-6 bg-zinc-900 border border-zinc-800 rounded-xl p-5">
            ...
            <div className="flex items-center gap-1.5">
              {(["parsing", "extracting", "translating", "rendering"] as JobState[]).map((stage) => {
                const stageIndex = (["pending", "parsing", "extracting", "translating", "rendering", "complete"] as JobState[]).indexOf(stage);
                const currentIndex = (["pending", "parsing", "extracting", "translating", "rendering", "complete"] as JobState[]).indexOf(job.state);
                const done = job.state === "complete" || currentIndex > stageIndex;
                const active = currentIndex === stageIndex;
                return (
                  <div
                    key={stage}
                    className={`flex-1 h-1 rounded-full ${
                      done ? "bg-green-500" : active ? "bg-blue-500 animate-pulse" : "bg-zinc-700"
                    }`}
                  />
                );
              })}
            ...
            {job.state === "complete" && (
              <a
                ...
                className="mt-4 flex items-center justify-center gap-2 w-full py-2.5 bg-green-600 hover:bg-green-500 rounded-lg text-sm font-medium transition-colors"
              >
